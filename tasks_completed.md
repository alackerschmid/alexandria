...
E3, E5, E6 and E7 shipped on `feat/e-block-polish` (branched off `fix/audit-process-hardening`). E1, E2 and E4 are still open in `tasks.md`.

Manual QA outstanding: all four touch frontend surfaces that carry no unit tests by project policy — the library search dropdown (E3), the scanner's title search and lookup (E5), the scanner session list and the guest banner (E7), and a screen-reader pass over the rating row (E7).
...
- **Implemented — one pass, taken before F1/F2 rather than after**, since the drift was already user-visible and the "To be implemented" list can be trimmed again when those land (noted in `tasks.md`'s sequencing).
