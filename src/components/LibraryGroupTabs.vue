<template>
  <div ref="containerRef" class="relative flex items-end w-full min-w-0 overflow-hidden">
    <span class="font-mono text-[9px] tracking-[0.2em] uppercase text-text-secondary/70 mr-3.5 pb-3 whitespace-nowrap">
      {{ $t('library.group_by') }}
    </span>
    ...
    >
      {{ opt.label }}
      <span v-if="opt.value === modelValue" class="text-orange-neon text-[10px]">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span>
    </button>

    <!-- Overflow "More" -->
    <v-menu v-if="overflowOptions.length" v-model="moreOpen" location="bottom end" offset="4">
      <template #activator="{ props: menuProps }">
        <button
          v-bind="menuProps"
          class="appearance-none bg-transparent cursor-pointer px-3.5 pt-2 pb-2.5 -mb-px inline-flex items-center gap-1.5 font-mono text-[11px] tracking-[0.14em] uppercase whitespace-nowrap transition-colors border-b-2"
          :class="moreActive ? 'text-orange-neon border-orange-neon' : 'text-text-secondary border-transparent hover:text-text-primary'"
        >
          {{ $t('library.more') }}
      ...
      <div class="bg-charcoal-light border border-charcoal-border shadow-xl py-1 min-w-[200px]">
        <button
          ...
          :class="opt.value === modelValue ? 'text-orange-neon' : 'text-text-secondary hover:text-text-primary hover:bg-white/[0.03]'"
          @click="pick(opt.value)"
        >
          {{ opt.label }}
          <span v-if="opt.value === modelValue" class="text-orange-neon text-[11px]">{{ sortDirection === 'asc' ? '↑' : '↓' }}</span>
        </button>
      </div>
    </v-menu>

    <!-- Hidden ghost row used to measure natural tab widths -->
    <div ref="ghostRef" aria-hidden="true" class="absolute top-0 left-0 flex items-end invisible pointer-events-none">
      <span class="font-mono text-[9px] tracking-[0.2em] uppercase mr-3.5 pb-3 whitespace-nowrap">{{ $t('library.group_by') }}</span>
      <span
...
interface Option { value: GroupBy; label: string }
...
function measure() {
  ...
  tabWidths.value = kids.slice(1).map(c => Math.ceil(c.getBoundingClientRect().width))
}

const visibleCount = computed(() => {
  const ws = tabWidths.value
  if (ws.length !== props.options.length || !availW.value) return props.options.length
  const fit = (reserve: number) => {
    let used = labelW.value
    let n = 0
    for (let i = 0; i < ws.length; i++) {
      const tail = i < ws.length - 1 ? reserve : 0
      if (used + ws[i] + tail <= availW.value) { used += ws[i]; n++ } else break
    }
    return n
  }
  const n1 = fit(0)
...
const visibleOptions = computed(() => props.options.slice(0, visibleCount.value))
const overflowOptions = computed(() => props.options.slice(visibleCount.value))
const moreActive = computed(() => overflowOptions.value.some(o => o.value === props.modelValue))

function pick(v: GroupBy) {
  moreOpen.value = false
  if (v === props.modelValue) emit('update:sortDirection', props.sortDirection === 'asc' ? 'desc' : 'asc')
...
watch(() => props.options.map(o => o.label).join('|'), () => nextTick(measure))
</script>
