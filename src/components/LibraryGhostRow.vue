<template>
  ...
  >
    ...
    <div class="flex-1 min-w-0 flex flex-col gap-1">
      <div
        class="font-heading text-sm font-bold text-text-secondary leading-snug line-clamp-2"
      >
        {{ title || "—"
        }}<span
          v-if="ordinal != null"
          class="font-normal text-text-secondary/60"
        >
          #{{ ordinal }}</span
        >
      </div>
      <div class="flex items-center gap-1 mt-auto pt-2">
        <span
          class="font-mono text-[10px] tracking-[0.12em] uppercase text-text-secondary/50"
        >
          {{ $t("library.unowned") }}
        </span>
        <span
