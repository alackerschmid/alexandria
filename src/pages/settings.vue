<template>
  <div class="bg-charcoal min-h-screen flex flex-col">
    ...
    <div class="flex flex-1 min-h-0">
      ...
      <!-- ── Desktop nav rail ──────────────────────────────────────────────── -->
      <aside class="hidden md:flex flex-col flex-none w-[280px] border-r border-charcoal-border py-8 sticky top-0 h-[calc(100vh-57px)] overflow-y-auto shrink-0">
        <h1 class="font-heading text-[30px] font-black text-text-primary px-7 leading-none mb-1">
          {{ $t('settings.heading') }}
        </h1>
        <p class="font-mono text-[10px] text-text-secondary/70 px-7 mb-7 truncate">
      ...
      <main class="flex-1 min-w-0 px-6 md:px-12 py-8 md:py-10 overflow-y-auto">
        <div class="max-w-[720px] flex flex-col gap-14">
          ...
          <!-- Mobile heading -->
          <div class="md:hidden">
            <h1 class="font-heading text-[34px] font-black text-text-primary leading-none">{{ $t('settings.heading') }}</h1>
            <p class="font-mono text-[10px] text-text-secondary/70 mt-2 truncate">
              {{ $t('settings.signed_in_as', { email: authStore.email }) }}
            </p>
          </div>
          ...
          <section id="account" ref="sectionRefs.account">
            <SectionHeading :title="$t('settings.account.heading')" :description="$t('settings.account.description')" />
            ...
            <div class="mt-5">
              ...
              <div v-else class="flex flex-col gap-4 max-w-xs">
                <SettingsField :label="$t('settings.account.current_password')">
                  <input v-model="passwordForm.current" type="password" class="settings-input" autocomplete="current-password" />
                </SettingsField>
                <SettingsField :label="$t('settings.account.new_password')">
                  <input v-model="passwordForm.next" type="password" class="settings-input" autocomplete="new-password" />
                </SettingsField>
                <SettingsField :label="$t('settings.account.confirm_password')">
                  <input v-model="passwordForm.confirm" type="password" class="settings-input" autocomplete="new-password" />
                </SettingsField>
            ...
            <div class="mt-6 flex items-center gap-4">
              ...
              <transition name="fade">
                <span v-if="accountSaved" class="font-mono text-[11px] text-text-secondary">
                  {{ $t('settings.account.saved') }} ✓
                </span>
              </transition>
              <span v-if="accountError" class="font-mono text-[11px]" style="color: rgb(var(--v-theme-error))">
                {{ accountError }}
              </span>
            </div>
          </section>
          ...
          <section id="accent" ref="sectionRefs.accent">
            <SectionHeading :title="$t('settings.accent.heading')" :description="$t('settings.accent.description')" />
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <button
                ...
                :style="{
                  background: preset,
                  border: accentStore.color === preset ? '2px solid rgb(var(--v-theme-on-background))' : '2px solid transparent',
                  boxShadow: accentStore.color === preset ? `0 0 0 1px ${preset}` : 'none',
                }"
                :title="preset"
              ...
              <!-- Native color picker -->
              <label class="inline-flex items-center gap-2.5 border border-charcoal-border px-3 py-[7px] cursor-pointer hover:border-orange-neon transition-colors">
                <span
                  class="w-5 h-5 rounded-full border border-white/15 flex-none"
                  :style="{ background: accentStore.color }"
                />
                <span class="font-mono text-[12px] text-text-primary uppercase tracking-[0.04em]">
                  {{ isPreset(accentStore.color) ? accentStore.color : $t('settings.accent.custom') }}
                </span>
                <input
                  type="color"
                  :value="accentStore.color"
                  class="w-0 h-0 opacity-0 absolute pointer-events-none"
                  @input="accentStore.set(($event.target as HTMLInputElement).value)"
                />
              </label>
            </div>
          </section>
          ...
          <section id="fields" ref="sectionRefs.fields">
            <SectionHeading :title="$t('settings.fields.heading')" :description="$t('settings.fields.description')" />

            <div class="border border-charcoal-border">
              <!-- Header row -->
              <div class="grid gap-0 px-4 border-b border-charcoal-border bg-charcoal-light" style="grid-template-columns: 1fr 120px 96px 40px">
                <div class="settings-col-head">{{ $t('settings.fields.col_name') }}</div>
                <div class="settings-col-head">{{ $t('settings.fields.col_type') }}</div>
                <div class="settings-col-head">{{ $t('settings.fields.col_required') }}</div>
                <div />
              </div>
              ...
              >
                <input
                  :value="def.name"
                  type="text"
                  class="bg-transparent border-none text-[14px] text-text-primary outline-none py-3 w-full"
                ...
                <div>
                  <button
                    ...
                    :style="{ background: def.required ? accentStore.color : 'rgba(138,128,120,0.3)' }"
                    @click="fieldDefsStore.update(def.id, { required: !def.required })"
                  >
                    <span
                      class="absolute top-0.5 w-4 h-4 rounded-full bg-charcoal transition-all duration-150"
                      :style="{ left: def.required ? 'calc(100% - 18px)' : '2px' }"
                    />
                  </button>
                </div>
              ...
              >
                <input
                  ...
                  class="bg-transparent border-none text-[14px] text-text-primary outline-none py-3 w-full placeholder:text-text-secondary/30"
                  @keydown.enter="confirmAddField"
                  @keydown.escape="addingField = false; newFieldName = ''"
                />
                <div>
                  <button
                    class="border border-charcoal-border/60 text-text-secondary font-mono text-[10px] tracking-[0.08em] uppercase px-2.5 py-1.5 hover:border-charcoal-border transition-colors"
            ...
            <div class="mt-4 flex items-center gap-4">
              <button
                ...
                @click="startAddField"
              >
                <span class="text-sm leading-none">+</span> {{ $t('settings.fields.add') }}
              </button>
              <v-btn
                v-if="addingField"
                variant="flat"
          ...
          <section id="export" ref="sectionRefs.export">
            ...
            <div class="flex items-center justify-between gap-6 border border-charcoal-border p-6">
              <div>
                <p class="text-[15px] text-text-primary font-medium">{{ $t('settings.export.description') }}</p>
              </div>
              <v-tooltip :text="$t('settings.export.coming_soon')" location="top">
                <template #activator="{ props: tipProps }">
                  <button
                    ...
                    :style="{ color: 'rgb(var(--v-theme-on-background))', borderColor: accentStore.color }"
                  >
          ...
          <section id="defaults" ref="sectionRefs.defaults">
            <SectionHeading :title="$t('settings.defaults.heading')" :description="$t('settings.defaults.description')" />

            <div class="flex flex-col">
              <DefaultRow :label="$t('settings.defaults.language')" :first="true">
                <SegControl
              ...
              <DefaultRow :label="$t('settings.defaults.theme')">
                <SegControl
                  :options="[
                    { value: 'light', label: $t('settings.defaults.theme_light') },
                    { value: 'dark',  label: $t('settings.defaults.theme_dark') },
                    { value: 'auto',  label: $t('settings.defaults.theme_auto') },
                  ]"
              ...
              <DefaultRow :label="$t('settings.defaults.view')">
                <SegControl
                  ...
                  @update:model-value="libraryDefaultsStore.setView($event as 'list' | 'tile')"
                />
              </DefaultRow>
              <DefaultRow :label="$t('settings.defaults.scan_status')">
                <SegControl
                  ...
                  @update:model-value="libraryDefaultsStore.setStatus($event as ReadStatus)"
                />
              </DefaultRow>
            </div>
          </section>
          ...
          <section id="danger" ref="sectionRefs.danger" class="pb-16">
            <div class="flex items-baseline gap-4 mb-[18px]">
              <span class="font-heading font-black text-[22px] leading-none" style="color: rgb(var(--v-theme-error))">
                {{ $t('settings.danger.heading') }}
              </span>
              <span class="flex-1 h-px" style="background: rgba(var(--v-theme-error), 0.25)" />
            </div>

            <div
              class="flex items-center justify-between gap-6 border p-6"
              style="border-color: rgba(var(--v-theme-error), 0.3); background: rgba(var(--v-theme-error), 0.04)"
            >
              <div>
                <p class="text-[15px] text-text-primary font-medium">{{ $t('settings.danger.delete_heading') }}</p>
                <p class="text-[12px] text-text-secondary mt-1 max-w-md leading-relaxed">{{ $t('settings.danger.delete_body') }}</p>
              </div>
              <button
                ...
                style="color: rgb(var(--v-theme-error)); border-color: rgb(var(--v-theme-error))"
    ...
    <v-dialog v-model="deleteDialog" max-width="400">
      <v-card rounded="0" :color="themeStore.isDark ? '#1c1b19' : '#ffffff'">
        <v-card-title class="font-heading text-xl pt-6 px-6 font-black text-text-primary" style="color: rgb(var(--v-theme-error))">
        ...
        <v-card-text class="px-6 text-sm text-text-secondary leading-relaxed">
          ...
          <div class="flex flex-col gap-3 mt-5">
            <SettingsField :label="$t('settings.danger.dialog_email_label')">
              <input v-model="deleteConfirmEmail" type="email" class="settings-input" autocomplete="off" />
            </SettingsField>
            <SettingsField :label="$t('settings.danger.dialog_password_label')">
              <input v-model="deleteConfirmPassword" type="password" class="settings-input" autocomplete="current-password" />
            </SettingsField>
            <p v-if="deleteError" class="text-[11px]" style="color: rgb(var(--v-theme-error))">{{ deleteError }}</p>
          </div>
        </v-card-text>
        <v-card-actions class="px-4 pb-4 gap-2">
          ...
          <v-btn
            ...
            @click="deleteDialog = false; deleteConfirmEmail = ''; deleteConfirmPassword = ''; deleteError = ''"
          >
          ...
          <v-btn
            ...
            :disabled="deleteConfirmEmail.toLowerCase() !== (authStore.email ?? '').toLowerCase() || !deleteConfirmPassword"
    ...
    <AppToast v-model="toast.show" :message="toast.message" :type="toast.type" :timeout="3500" />
  </div>
</template>

<script lang="ts" setup>
import { ref, reactive, nextTick, onMounted, onUnmounted, defineComponent, h } from 'vue'
...
const NAV_SECTIONS = [
  { id: 'account', get label() { return t('settings.account.heading') } },
  { id: 'accent',  get label() { return t('settings.accent.heading')  } },
  { id: 'fields',  get label() { return t('settings.fields.heading')  } },
  { id: 'export',  get label() { return t('settings.export.heading')  } },
  { id: 'defaults',get label() { return t('settings.defaults.heading') } },
  { id: 'danger',  get label() { return t('settings.danger.heading')  } },
]
...
const ACCENT_PRESETS = ['#ff6600', '#d9534f', '#d4a017', '#2d8f4e', '#3b6dde', '#9b4dca']
...
async function saveAccount() {
  ...
  if (trimmedName && trimmedName !== (authStore.firstname ?? '')) body.firstname = trimmedName
  const trimmedEmail = accountForm.email.trim().toLowerCase()
  if (trimmedEmail && trimmedEmail !== (authStore.email ?? '').toLowerCase()) body.email = trimmedEmail
  ...
  try {
    ...
    const data = await res.json() as { firstname?: string; email?: string; passwordChanged?: boolean; error?: string }
    if (!res.ok) {
      ...
      savedTimer = setTimeout(() => { accountSaved.value = false }, 2500)
    }
  } catch {
...
function cycleFieldType(def: { id: number; type: string }) {
  const next = FIELD_TYPES[(FIELD_TYPES.indexOf(def.type) + 1) % FIELD_TYPES.length]
  fieldDefsStore.update(def.id, { type: next })
}

async function onFieldNameBlur(def: { id: number; name: string }, e: FocusEvent) {
...
async function deleteField(id: number) {
  try {
    ...
    if (res.ok) fieldDefsStore.remove(id)
  } catch {}
}
...
function cycleNewFieldType() {
  newFieldType.value = FIELD_TYPES[(FIELD_TYPES.indexOf(newFieldType.value) + 1) % FIELD_TYPES.length]
}
...
const toast = reactive({ show: false, message: '', type: 'error' as 'error' | 'success' })
  ...
  setup(props) {
    return () => h('div', { class: 'mb-6' }, [
      h('div', { class: 'flex items-baseline gap-4 mb-1.5' }, [
        h('span', { class: 'font-heading font-black text-[22px] text-text-primary leading-none' }, props.title),
        h('span', { class: 'flex-1 h-px bg-charcoal-border' }),
      ]),
      props.description
        ? h('p', { class: 'text-[13px] text-text-secondary max-w-lg leading-relaxed' }, props.description)
        : null,
    ])
  },
})
...
const SettingsField = defineComponent({
  props: { label: String },
  setup(props, ctx) {
    return () => h('label', { class: 'flex flex-col gap-2' }, [
      h('span', { class: 'font-mono text-[10px] tracking-[0.16em] uppercase text-text-secondary' }, props.label),
      ctx.slots.default?.(),
    ])
  },
})
  ...
  setup(props, { emit }) {
    return () => h('div', {
      ...
      h('button', {
        ...
        style: opt.value === props.modelValue
          ? { background: accentStore.color, color: '#111110', fontWeight: 700 }
          : { background: 'transparent', color: 'rgb(var(--v-theme-text-secondary))' },
      }, opt.label),
    ))
  },
})
