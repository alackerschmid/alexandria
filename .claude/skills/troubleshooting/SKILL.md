...
- **In production**, use the `cloudflare-observability` MCP server instead — `wrangler dev` logs say nothing about the deployed worker. Query `query_worker_observability` with `$metadata.service = bookscan-worker` over the last few hours and check, in order: (1) that scheduled invocations exist at all (`$metadata.origin` / `$metadata.trigger` = the cron trigger — if they're missing the sweeper isn't running, which is a different bug from a slow sweeper); (2) SPARQL error and timeout counts; (3) the per-tick telemetry from `worker/src/enrichment.ts`. Run `observability_keys` first to confirm field names — retention is 7 days, so a backlog older than that leaves no trace.
