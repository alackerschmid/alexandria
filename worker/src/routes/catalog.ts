...
import {
  ...
  fetchOpenLibraryEditionsByWorkId,
...
async function loadEditions(db: D1Database, userId: number, workId: string) {
  ...
  const work = await db
    .prepare("SELECT editions_checked_at FROM works WHERE id = ?")
    .bind(workId)
...
works.post("/:workId/editions/discover", async (c) => {
  ...
  const work = await db
    .prepare(
      "SELECT editions_checked_at, openlibrary_work_id FROM works WHERE id = ?",
    )
    .bind(workId)
    .first<{
      editions_checked_at: string | null;
  ...
  if (!work.editions_checked_at) {
    ...
    let related = seed ? await fetchOpenLibraryEditions(seed.isbn, usage) : [];
...
series.get("/", async (c) => {
  ...
  const byId: Record<
    number,
    { id: number; name: string | null; entries: unknown[] }
  > = {};
  for (const r of rows) {
    const { series_id, series_name, ...entry } = r;
    if (!byId[series_id])
      byId[series_id] = { id: series_id, name: series_name, entries: [] };
