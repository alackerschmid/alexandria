...
type CustomFieldRow = { field_def_id: number; field_name: string; field_type: string; field_value: string | null }
...
stats.get('/', async (c) => {
  ...
  const [{ results }, seriesResult, customFieldResult] = await Promise.all([
    c.env.DB.prepare(`
    ...
    c.env.DB.prepare(`
      ...
      ORDER BY count DESC
      LIMIT 15
    `).bind(locale, userId).all<SeriesRow>(),
    ...
    `).bind(userId).all<CustomFieldRow>(),
  ])
  ...
  for (const r of rows) {
    if (r.publisher) publisherCounts.set(r.publisher, (publisherCounts.get(r.publisher) ?? 0) + 1)
  }
  ...
  for (const r of rows) {
    if (r.form_of_work) formCounts.set(r.form_of_work, (formCounts.get(r.form_of_work) ?? 0) + 1)
  }
  ...
  for (const r of rows) {
    if (r.main_subject) subjectCounts.set(r.main_subject, (subjectCounts.get(r.main_subject) ?? 0) + 1)
  }
  ...
  // Series
  const topSeries = seriesResult.results.map(r => ({ label: r.label, count: r.count }))

  // Custom fields — group by field def, count values
  const cfMap = new Map<number, { fieldName: string; fieldType: string; valueCounts: Map<string, number> }>()
  for (const row of customFieldResult.results) {
    if (row.field_value == null) continue
    if (!cfMap.has(row.field_def_id)) {
      cfMap.set(row.field_def_id, { fieldName: row.field_name, fieldType: row.field_type, valueCounts: new Map() })
    }
    const entry = cfMap.get(row.field_def_id)!
    const vals = row.field_type === 'tag' ? parseTagArray(row.field_value) : [row.field_value]
    for (const v of vals) {
      entry.valueCounts.set(v, (entry.valueCounts.get(v) ?? 0) + 1)
    }
  }
  const customFields = [...cfMap.entries()].map(([fieldDefId, { fieldName, valueCounts }]) => ({
    fieldDefId,
    fieldName,
