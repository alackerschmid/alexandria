...
**`book_overrides`** — per-user field overrides: `user_id` → `users`, `book_id` → `books`, `title`, `cover_url`, `language`, `publish_date`, `number_of_pages_median`, `description`, `publisher`, `edition_name` (all nullable), `updated_at`. Unique on `(user_id, book_id)`. `author` is deliberately absent (dropped in 0009) — authors are normalized into `authors`/`work_authors`, so a flat per-user string would be invisible to every display and search path that reads them. A NULL column means "no override": `buildScanSelect` COALESCEs it against `books`, and clearing a field writes NULL rather than an empty string.
