...
import { hashPassword, verifyPassword, isLegacyHash, needsRehash } from '../src/password'
...
describe('verifyPassword (pbkdf2)', () => {
  ...
  it('rejects malformed stored hashes without throwing', async () => {
    ...
    expect(await verifyPassword('pw', 'pbkdf2$notanumber$AAAA$AAAA')).toBe(false)
...
describe('needsRehash', () => {
  ...
  it('flags a pbkdf2 hash minted with fewer iterations than the current setting', async () => {
    const stale = (await hashPassword('pw')).replace(/^pbkdf2\$\d+\$/, 'pbkdf2$50000$')
